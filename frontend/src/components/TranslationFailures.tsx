...
export default function TranslationFailures({ jobId, stopped }: { jobId: string; stopped: boolean }) {
  ...
  return <section className="my-4 w-full min-w-0 rounded-lg border border-amber-300 bg-amber-50 p-4">
    ...
    <p>{stopped ? '成功片段已保留。可在此粘贴人工译文；正文片段也可先标记为待审阅补译，再从检查点恢复进入审阅。' : '后续内容仍在处理。请等待结束或先暂停任务，再处理以下片段。'}待补译内容完成审阅前不能导出。</p>
    ...
    {Object.entries(ledger.items).map(([key, item]) => <details key={key} className="my-3">
      ...
      <p className="text-sm text-red-700">{/(new_sensitive|content_filter)/i.test(item.error)
      ...
      {!key.startsWith('footnote:') && /(new_sensitive|content_filter)/i.test(item.error) && <button className="ml-3 rounded border border-amber-500 bg-white px-3 py-2 text-amber-950 disabled:opacity-50" disabled={!stopped || saving || Boolean(drafts[key]?.trim())} onClick={() => save(key, 'defer_to_review')}>留到审阅时补译</button>}
      {!key.startsWith('footnote:') && /(new_sensitive|content_filter)/i.test(item.error) && <p className="my-2 text-sm text-amber-950">选择后请使用恢复按钮继续处理本书。审阅中会保留原文作为待补译提示；这不算翻译成功，补齐中文并批准前不能导出。</p>}
      ...
      <button disabled={!stopped || saving || !reasons[key]?.trim()} onClick={() => save(key, 'preserve_source')}>明确保留原文（非翻译成功）</button>
    </details>)}
  </section>
}
