...
describe('Review draft navigation', () => {
  ...
  test('offers optional link hints without blocking export', async () => {
    ...
    render(<Review />)
    expect(await screen.findByText('标题、列表、表格、图片或书内链接仍和原文不完全一致。润色时已经交给模型处理过，导出不会再改。')).toBeTruthy()
    expect(screen.queryByText('chapter.xhtml#note-2')).toBeNull()
    expect(screen.queryByRole('button', { name: '查看对应段落' })).toBeNull()
