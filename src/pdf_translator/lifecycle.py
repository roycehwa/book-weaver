...
def _select_final_markdown(run_dir: Path) -> Path | None:
...
def finalize_run(run_dir: Path) -> dict[str, Any]:
    ...
    final_markdown = _select_final_markdown(run_dir)
    ...
    status = {
        ...
        "ready_for_phase_b": not required_missing and (run_dir / "book.json").exists(),
    }
